...
function createStore() {
  ...
  return {
    ...
    // Append a new message (immutable) and broadcast an "add" event to all clients
    add(content, device) {
    ...
    // Remove a message by id (immutable) and broadcast a "delete" event.
    // Returns true if a message was removed, false if the id was not found.
    remove(id) {
      const exists = messages.some((m) => m.id === id);
      if (!exists) return false;
      messages = messages.filter((m) => m.id !== id); // immutable update
